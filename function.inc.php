...
function send_email($email,$html,$subject){
  ...
  $mail->SMTPSecure="tls";
  $mail->SMTPAuth=true;
  $mail->Username="medulla1230@gmail.com";
  $mail->Password="ademola4707";
  $mail->setFrom("medulla1230@gmail.com");
...
function manageWallet($uid,$amt,$type,$msg,$close_time,$payment_id=''){
  global $con;
  $added_on=date('Y-m-d h:i:s');
  $sql="insert into wallet(user_id,amt,msg,type,added_on,payment_id,close_time) values('$uid','$amt','$msg','$type','$added_on','$payment_id','$close_time')";
  $res=mysqli_query($con,$sql);
}
...
function incomingpPL(){
  global $con;
  $sql="select * from fxrp_trades where type = '0' or type = '1'  ORDER by close_time DESC LIMIT 1 ";
  $res=mysqli_query($con,$sql);
   while($rows=mysqli_fetch_assoc($res)){

    return  $rows['net_profit'];
...
function CloseTime(){
  global $con;
  $sql="select * from fxrp_trades ORDER by close_time DESC LIMIT 1 ";
  $res=mysqli_query($con,$sql);
   while($rows=mysqli_fetch_assoc($res)){
...
function chkprofitloss($uid){
  global $con;
  $sql="select * from wallet where user_id='$uid' AND msg = 'tradesPL' order by id desc ";
  $res=mysqli_query($con,$sql);
  $in = 0;
  while($row=mysqli_fetch_assoc($res)){
     if($row['type']=='in'){
